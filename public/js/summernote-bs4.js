  ...
  VideoDialog_createClass(VideoDialog, [{
    ...
    value: function createVideoNode(url) {
      ...
      if (ytMatch && ytMatch[1].length === 11) {
        ...
        $video = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<iframe>').attr('frameborder', 0).attr('src', '//www.youtube.com/embed/' + youtubeId + (start > 0 ? '?start=' + start : '')).attr('width', '100%').attr('height', '350');
      } else if (igMatch && igMatch[0].length) {
        $video = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<iframe>').attr('frameborder', 0).attr('src', 'https://instagram.com/p/' + igMatch[1] + '/embed/').attr('width', '100%').attr('height', '400').attr('scrolling', 'no').attr('allowtransparency', 'true');
